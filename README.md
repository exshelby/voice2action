...
The local operations dashboard is available at `http://localhost:3000/operations` while `npm run dev` is running. It shows ticket totals, recent tickets, assigned teams, status progress, and pending notification messages. Search by ticket reference or wording, or filter the list by status, team, and category. Status changes advance one step at a time and require checking a confirmation box. Open a ticket to view its full transcript and review audit trail, change its assigned team, or confirm that a pending local notification was delivered. Manual reassignment refreshes the existing assignment notification for the new team. For safety, the dashboard and its mutation actions accept localhost requests only; authentication is required before any future public deployment.
